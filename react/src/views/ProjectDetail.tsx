    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <button
            onClick={() => navigate(`/projects/${projectId}/thread`)}
            style={{
              padding: "8px 16px",
          ...
          <button
            ...
            style={{
              padding: "8px 16px",
              backgroundColor: activeTab === "manage" ? "rgba(0,255,136,0.1)" : "transparent",
              border: "1px solid #2f2f2f",
              borderBottom: activeTab === "manage" ? "2px solid #00ff88" : "1px solid #2f2f2f",
              color: activeTab === "manage" ? "#00ff88" : "#8a8a8a",
              fontSize: 12,
              cursor: "pointer",
              fontFamily: "inherit",
            }}
...
function ProjectManagePanel({
  ...
  allModels: Array<{ ref: string; label: string }>;
  onUpdate: (data: Partial<{ name: string; path: string; model: string }>) => Promise<void>;
}) {
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 32 }}>
      <div style={{ maxWidth: 640, display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ padding: 20, backgroundColor: "#141414", border: "1px solid #2f2f2f" }}>
          <div style={{ fontSize: 12, fontWeight: "bold", color: "#fff", marginBottom: 16, textTransform: "uppercase", letterSpacing: 1 }}>项目信息</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <button
              onClick={() => {
                setSaving(true);
                void onUpdate({ name, path, model }).then(() => setSaving(false));
              }}
              disabled={saving}
              style={{
                marginTop: 8,
